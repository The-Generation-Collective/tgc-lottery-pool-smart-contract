...
// @author: @ass77
// @team: https://instagram.com/tgcollective777
...
contract LotteryRexx {
    ...
    function DrawWinnerTicket() public isOperator {
        ...
        // if user owns at least 1 REXX token, increase the odds by 5%
        if (rexxieOwner.balanceOf(tickets[winningTicket]) > 0) {
            winningTicket = (winningTicket + 1) % tickets.length;
        }

        // TODO if user owns more than 2 REXX tokens, increase the odds by 10%
        if (rexxieOwner.balanceOf(tickets[winningTicket]) > 2) {
            winningTicket = (winningTicket + 1) % tickets.length;
        }

        // TODO if user owns more than 6 REXX tokens, increase the odds by 30%
        if (rexxieOwner.balanceOf(tickets[winningTicket]) > 6) {
            winningTicket = (winningTicket + 1) % tickets.length;
        }

        address winner = tickets[winningTicket];
        lastWinner = winner;ASDASDASD!!
